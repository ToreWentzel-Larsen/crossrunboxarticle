...
\abstract{%
Run charts is one form of statistical process control charts that is
...
(L) of data points on the same side of the proces centre (mean or
median) and unusually few crossings (C) of the centre line. The critical
...
specificity) of the Anhøj rules also depend on N and have a rugget
...
recently released an R package, \CRANpkg{crossrun} \citep{twl2018}, that
...
\code{exp(log-likelihood)}. The limits and specificities are presented
...
user to choose between \code{"anhoej"}, \code{"bestbox"}, and
